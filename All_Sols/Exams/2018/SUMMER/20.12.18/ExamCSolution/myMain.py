...
if qn == 1:
    ...
    print myDashMatrix(3), '\n'
    print myDashMatrix(4)

elif qn == 2:

    myStr = 'Hello Hello We Do Hi no No no we Donot Hey HE he heY 123'

    print countMyStrings(myStr)
    ...
    myData0 = np.concatenate((myMat, myMat), axis=0)
    myData = np.concatenate((myData0, myData0), axis=1)
    Field = myLinearDependancyRemoval(myData)
    ...
    newImg = myHalfEdgeMaskKernel(img)
